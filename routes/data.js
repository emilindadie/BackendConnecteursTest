...
async function login(){
   const loginResponse = await MainController.login();
   return loginResponse;
}

async function getAccessToken(refresh_token){
   const accessTokenResponse = await MainController.getAccessToken(refresh_token);
   return accessTokenResponse;
}

async function recursiveGetAccounts(access_token){
   const accounts = [];
   let userAccountsResponse = await MainController.getRecursiveData(access_token, `${process.env.API_HOST}:${process.env.API_PORT}/accounts`);
   ...
   while(userAccountsResponse.data.link !== null && userAccountsResponse.data.link.next !== null){
      userAccountsResponse = await MainController.getRecursiveData(access_token, `${process.env.API_HOST}:${process.env.API_PORT}${userAccountsResponse.data.link.next}`);
      accounts.push(...userAccountsResponse.data.account);
   }
   return accounts;
}
...
async function recursiveGetTransactions(access_token, acc_number){
   const transactions = [];
   let accountTransactionsResponse = await MainController.getRecursiveData(access_token, `${process.env.API_HOST}:${process.env.API_PORT}/accounts/${acc_number}/transactions`);
   ...
   while(accountTransactionsResponse.data.link !== null && accountTransactionsResponse.data.link.next !== null){
      accountTransactionsResponse = await MainController.getRecursiveData(access_token, `${process.env.API_HOST}:${process.env.API_PORT}${accountTransactionsResponse.data.link.next}`);
      transactions.push(...accountTransactionsResponse.data.transactions);
   }
   return transactions;
}
